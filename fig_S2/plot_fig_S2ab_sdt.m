function plot_fig_S2ab_sdt(path)

hexcolors = {'#EFB700', '#B81D13'};

colors2use = NaN(2,3);
colorBins  = [2:3; 4:5; 6:7];

for iHex = 1:numel(hexcolors)
    for iBin = 1:size(colorBins)
        colors2use(iHex,iBin) = hex2dec(hexcolors{iHex}(colorBins(iBin,1):colorBins(iBin,end)));
    end
end

% scale by dividing through 256 (Matlab conversion)
colors2use = colors2use/ 256;

%% Plot d-prime

tbl = readtable(fullfile(path.data, 'SourceData.xlsx'), 'sheet', 'Fig_S2a');

ga_25 = tbl.Var1(2:end);
ga_75 = tbl.Var2(2:end);

figure;

% data to plot
plot_dprime = [ga_25, ga_75];

jw_raincloud_pairplot(plot_dprime, {colors2use(1,:), colors2use(2,:)});

set(gca, 'ytick', []);
xlabel('d-prime');

h = gca; h.YAxis.Visible = 'off';

set(gca, 'fontsize', 13);

box off

%% Plot criterion

tbl = readtable(fullfile(path.data, 'SourceData.xlsx'), 'sheet', 'Fig_S2b');

ga_25 = tbl.Var1(2:end);
ga_75 = tbl.Var2(2:end);

figure;

% data to plot
plot_dprime = [ga_25, ga_75];

jw_raincloud_pairplot(plot_dprime, {colors2use(1,:), colors2use(2,:)});

set(gca, 'ytick', []);
xlabel('criterion');

h = gca; h.YAxis.Visible = 'off';

set(gca, 'fontsize', 13);

box off
