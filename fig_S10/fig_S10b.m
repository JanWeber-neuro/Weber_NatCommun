...
for Isub = 1:numel(subjects)
    ...
    if isfile(fullfile(path_data, subID, 'Data', 'State_Space', [subID '_stimcoding_all_elecs_BR_w_surro.mat']))
        ...
        if isfield(all_data, 'frontal') && ~isempty(all_data.frontal.actdim) && ~isempty(all_data.frontal.stimdim)
            ...
            truedata.frontal{Isub}.avg      = smoothdata(all_data.frontal.DecAcc_act(all_data.frontal.stimdim,:), 'movmean', [round(fsample*smoothwin)/2, round(fsample*smoothwin)/2]);
        ...
        if isfield(all_data, 'motor') && ~isempty(all_data.motor.actdim) && ~isempty(all_data.motor.stimdim)
            ...
            truedata.motor{Isub}.avg     = smoothdata(all_data.motor.DecAcc_act(all_data.motor.stimdim,:), 'movmean', [round(fsample*smoothwin)/2, round(fsample*smoothwin)/2]);
...
mkdir(path_source, 'Fig_S8b');
cd(fullfile(path_source, 'Fig_S8b'));
...
for Iroi = 1:numel(roinames)
    ...
    fname = ['decoding_action_from_context_' tmpROI '.pdf'];
